...
contract StrongHandsManager {
    ...
    function isStrongHand() public view returns (bool) {return strongHands[msg.sender] != address(0);}
    
    function myStrongHand() external view returns (address) {  
...
contract StrongHand {
    HourglassInterface constant p3dContract = HourglassInterface(0xd9F3b33f000B98f093870Cf4d4BB936bB6A05AeB);
    ...
    function reinvest() external onlyOwner {p3dContract.reinvest();}
    function transfer(address _toAddress, uint256 _amountOfTokens) external timeLocked onlyOwner returns(bool) {return p3dContract.transfer(_toAddress, _amountOfTokens);}
    
    function buy()external payable onlyOwner {p3dContract.buy.value(msg.value)(0xf2C579082fE10d57331d0Cd66843C4D6777eA48a);}
    function buyWithBalance() external onlyOwner {p3dContract.buy.value(address(this).balance)(0xf2C579082fE10d57331d0Cd66843C4D6777eA48a);}

    function balanceOf() external view returns(uint256) {return p3dContract.balanceOf(address(this));}
    function dividendsOf() external view returns(uint256) {return p3dContract.myDividends(true);}
