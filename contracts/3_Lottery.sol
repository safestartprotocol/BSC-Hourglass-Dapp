...
contract DailyLottery {
    ...
    function isRoundOver() public view returns(bool) {return now >= rounds[currentRoundNumber].startTime.add(ROUND_LENGTH);}
    
    function potentialWinner() external view returns(address) {
        if(isRoundOver() && rounds[currentRoundNumber].blockNumber != 0 &&
    ...
    function blocksUntilNewPotentialWinner() external view returns (uint256) {
        if(isRoundOver() &&
    ...
    function timeLeft() external view returns(uint256) {
        if(isRoundOver()) {return 0;}
        return ROUND_LENGTH.sub(now.sub(rounds[currentRoundNumber].startTime));
    }
    
    function getTicketOwner(uint256 _number) external view returns(address) {return rounds[currentRoundNumber].tickets[_number];}
    function ticketsPurchased() external view returns(uint256) {return rounds[currentRoundNumber].ticketsSold;}
    
    function jackpotSize() external view returns(uint256) {return rounds[currentRoundNumber].pot.add(p3dContract.myDividends(true)).mul(97) / 100;}
    function validatorReward() external view returns(uint256) {return rounds[currentRoundNumber].pot.add(p3dContract.myDividends(true)) / 100;}
    
    function myVault() external view returns(uint256) {return vault[msg.sender];}
    function myTickets() external view returns(uint256) {return rounds[currentRoundNumber].ticketsPerAddress[msg.sender];}
    
    function purchaseTicket(uint256 _howMany) external payable isValidPurchase(_howMany) {
    ...
    function purchaseFromVault(uint256 _howMany) external canPayFromVault(_howMany) {
        if(!isRoundOver() || onRoundEnd()) {
    ...
    function withdrawFromVault() external positiveVaultBalance {
    ...
    function onRoundEnd() private returns(bool newRound) {
    ...
    function acceptPurchase(uint256 _howMany, uint256 value) private {
        ...
        p3dContract.buy.value(value.mul(40) / 100)(dev);
    }
}
